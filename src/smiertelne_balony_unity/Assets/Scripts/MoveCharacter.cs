...
public class MoveCharacter : MonoBehaviour {
    ...
    public void moveCamera() {
        if ( !objectPrecisionPositioning( transform, waypoints[current_position], 0.1f ) ) {
            //var _direction = (waypoints[current_position].position - transform.position).normalized;
            //var _lookRotation = Quaternion.LookRotation(_direction);
            //transform.rotation = Quaternion.Slerp(transform.rotation, _lookRotation, Time.deltaTime * rotation * Time.deltaTime);
